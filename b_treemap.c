...
EXPORT struct b_treemap_t* b_treemap_new_int(void (*freeValueFunc)(void*)) {
...
EXPORT b_treemap_error_t b_treemap_put_int(struct b_treemap_t* data, int key, void* value) {
	void* p = 0;
	p += key;
...
EXPORT void* b_treemap_get_int(struct b_treemap_t* data, int key) {
	void* p = 0;
	p += key;
